<?php

namespace App\Models;

use Eloquent as Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\SoftDeletes;
use Spatie\Activitylog\Traits\LogsActivity;

/**
 * Class subestacao
 * @package App\Models
 * @version May 24, 2020, 9:01 am UTC
 *
 * @property integer idcidade
 * @property string subestacao
 * @property string transformador
 * @property string instalacao
 * @property string barramento
 * @property string bairro
 * @property string tensao_secundaria
 * @property string fases
 * @property string kvan
 * @property string|\Carbon\Carbon incluidoem
 * @property integer idusuario
 */
class subestacao extends Model
{
    use SoftDeletes;
    use LogsActivity;

    public $table = 'subestacaos';

    const CREATED_AT = 'created_at';
    const UPDATED_AT = 'updated_at';


    protected $dates = ['deleted_at'];


    protected static $logAttributes = ['*'];



    public $fillable = [
        'idcidade',
        'subestacao',
        'transformador',
        'instalacao',
        'barramento',
        'bairro',
        'tensao_secundaria',
        'fases',
        'kvan',
        'incluidoem',
        'idusuario'
    ];

    /**
     * The attributes that should be casted to native types.
     *
     * @var array
     */
    protected $casts = [
        'id' => 'integer',
        'idcidade' => 'integer',
        'subestacao' => 'string',
        'transformador' => 'string',
        'instalacao' => 'string',
        'barramento' => 'string',
        'bairro' => 'string',
        'tensao_secundaria' => 'string',
        'fases' => 'string',
        'kvan' => 'string',
        'incluidoem' => 'datetime',
        'idusuario' => 'integer'
    ];

    /**
     * Validation rules
     *
     * @var array
     */
    public static $rules = [
        'idcidade' => 'required',
        'subestacao' => 'required',
        'transformador' => 'required',
        'instalacao' => 'required',
        'incluidoem' => 'required',
        'idusuario' => 'required'
    ];

    public function usuario(): BelongsTo
    {
        return $this->belongsTo(usuario::class, 'idusuario');
    }
    public function cidade(): BelongsTo
    {
        return $this->belongsTo(cidade::class, 'idcidade');
    }
    public  function relationsName(){
        $reflector = new \ReflectionClass($this);
        $relations = [];
        foreach ($reflector->getMethods() as $reflectionMethod) {
            $returnType = $reflectionMethod->getReturnType();
            if ($returnType) {
                if (in_array(class_basename($returnType->getName()), ['HasOne', 'HasMany', 'BelongsTo', 'BelongsToMany', 'MorphToMany', 'MorphTo'])) {
                    $relations[] = $reflectionMethod->name;
                }
            }
        }

        return $relations;
    }
}
